...
public class Draggable : MonoBehaviour
{
    ...
    IEnumerator AbsorbIntoHole()
    {
        ...
        if (treeGrowthScript != null)
        {
            ...
            // ตรวจสอบทุก stage ใน growthStages ว่ามี active อยู่หรือไม่ และจ่ายเหรียญตาม coinRewardsPerStage
            for (int i = 0; i < treeGrowthScript.growthStages.Length; i++)
            {
                if (treeGrowthScript.growthStages[i].activeSelf)
                {
                    // ตรวจสอบว่ามีราคากำหนดไว้ไหม (index ไม่เกิน array)
                    if (i < coinRewardsPerStage.Length)
                    {
                        ...
                        if (reward > 0)
                        {
                            ...
                            coinAdded = true;
                            break; // เจอ stage ที่ active แล้วจ่ายเหรียญแล้วจบ
                        }
                    }
                }
            }
